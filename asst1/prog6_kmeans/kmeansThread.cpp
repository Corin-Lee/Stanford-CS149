...
using namespace std;
atomic<int> point_index_atomic;
constexpr unsigned int thread_nums = 32;
typedef struct {
...
void computeAssignmentsNew(WorkerArgs* const args) {
  double minDist = 1e30;
  int span = 1024;
  int point_start = point_index_atomic.fetch_add(span);
  while (point_start < args->M) {
    for (int i = 0; i < span && point_start + i < args->M; ++i) {
      int point_index = point_start + i;
      args->clusterAssignments[point_index] = -1;
      // Assign datapoints to closest centroids
      for (int k = args->start; k < args->end; k++) {
        double d = dist(&args->data[point_index * args->N],
                        &args->clusterCentroids[k * args->N], args->N);
        if (d < minDist) {
          minDist = d;
          args->clusterAssignments[point_index] = k;
        }
      }
      minDist = 1e30;
    }
    point_start = point_index_atomic.fetch_add(span);
  }
}
/**
...
void kMeansThreadNew(double* data, double* clusterCentroids,
                     int* clusterAssignments, int M, int N, int K,
                     double epsilon) {
  // Used to track convergence
  double* prevCost = new double[K];
  ...
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    std::thread workers[thread_nums];
    for (unsigned int i = 0; i < thread_nums; ++i) {
      workers[i] = std::thread(computeAssignmentsNew, &args);
    }
