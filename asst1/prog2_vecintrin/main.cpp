...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  float max_float = 9.999999f;
  __cs149_mask all_one_mask = _cs149_init_ones();
  __cs149_mask all_zero_mask = _cs149_mask_not(all_one_mask);
  __cs149_vec_int all_zero, all_one;
  __cs149_vec_float all_max_float;
  _cs149_vset_int(all_zero, 0, all_one_mask);
  _cs149_vset_int(all_one, 1, all_one_mask);
  _cs149_vset_float(all_max_float, max_float, all_one_mask);

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    __cs149_mask cur_mask = _cs149_init_ones(N - i);
    __cs149_vec_float xs;
    __cs149_vec_int ys;
    __cs149_vec_float results;
    _cs149_vset_float(results, 1.0f, all_one_mask);

    float* values_p = values + i;
    int* exponents_p = exponents + i;
    float* output_p = output + i;
    _cs149_vload_float(xs, values_p, cur_mask);
    _cs149_vload_int(ys, exponents_p, cur_mask);

    // calculation
    __cs149_mask mul_mask = all_one_mask;
    _cs149_vgt_int(mul_mask, ys, all_zero, cur_mask);
    while (_cs149_cntbits(mul_mask) > 0) {
      _cs149_vmult_float(results, xs, results, mul_mask);
      _cs149_vsub_int(ys, ys, all_one, mul_mask);
      _cs149_vgt_int(mul_mask, ys, all_zero, cur_mask);
    }

    __cs149_mask write_max_float = all_zero_mask;
    _cs149_vgt_float(write_max_float, results, all_max_float, cur_mask);
    _cs149_vset_float(results, max_float, write_max_float);
    _cs149_vstore_float(output_p, results, cur_mask);
  }
}
...
float arraySumVector(float* values, int N) {
  __cs149_mask all_one_mask = _cs149_init_ones();
  __cs149_vec_float cur_sums;
  _cs149_vset_float(cur_sums, 0.0f, all_one_mask);
  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    __cs149_vec_float cur_nums;
    _cs149_vload_float(cur_nums, values + i, all_one_mask);
    _cs149_vadd_float(cur_sums, cur_sums, cur_nums, all_one_mask);
  }

  for (int t = VECTOR_WIDTH; t > 1; t /= 2) {
    _cs149_hadd_float(cur_sums, cur_sums);
    _cs149_interleave_float(cur_sums, cur_sums);
  }
  return cur_sums.value[0];
}
